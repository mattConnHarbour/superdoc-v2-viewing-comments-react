...
import type { SuperDocRef, SuperDocInstance, DocumentMode } from '@superdoc-dev/react';
...
const commentsConfig = { visible: true } as const;
const commentsUiConfig = { comments: { displayMode: 'sidebar' } } as const;
const modulesConfig = { comments: {} } as const;
...
declare global {
  interface Window {
    __superdocDemoReadyCount?: number;
    __superdocDemoInstance?: SuperDocInstance;
  }
}
...
function App() {
  // Document state
  const [document, setDocument] = useState<File | string>('/test_file.docx');
  ...
  return (
    <div className="app">
      ...
      <header className="header">
        <h1>SuperDoc 2.8.0 — viewing-mode comments patch</h1>
        ...
        {document ? (
          <SuperDocEditor
            ...
            onReady={({ superdoc }) => {
              ...
              window.__superdocDemoReadyCount =
                (window.__superdocDemoReadyCount ?? 0) + 1;
              window.__superdocDemoInstance = superdoc;
              setIsReady(true);
            }}
